...
export function useCookies<T>(
  ...
  useEffect(() => {
    const restoredValue: T = getCookieStorage(key);
  ...
  const setState = useCallback(
    (updateState: T | Partial<T>) => {
      if (multiValue) {
        set((prevValue) => {
          setCookieStorage<T>(key, { ...prevValue, ...updateState }, options?.daysUntilExpiration);
          return { ...prevValue, ...updateState };
        });
      } else {
        setCookieStorage<T>(key, updateState as T, options?.daysUntilExpiration);
...
export function setCookieStorage<T>(key: string, value: T, daysUntilExpiration: number = 0) {
  try {
...
export function removeCookieStorage(key: string) {
  try {
